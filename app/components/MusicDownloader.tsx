...
import MusicPreview, { Playlist } from "@/app/components/MusicPreview";
...
export function MusicDownloader() {
  ...
  const searchPlaylist = async (url: string) => {
    ...
    if (!playlistId) throw new Error("⚠️ Link inválido.");
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!url) return setAlert("emptyLink");
    if (!url.toLowerCase().includes("spotify")) return setAlert("incorrectLink");
  ...
  return (
    <div className="h-screen bg-gradient-to-b from-gray-950 via-gray-900 to-black relative overflow-hidden flex items-center justify-center px-4">
      ...
      {alert && (
        <motion.div initial={{ y: -100, opacity: 0 }} animate={{ y: 0, opacity: 1 }} exit={{ y: -100, opacity: 0 }} className="fixed top-[2.5rem] left-1/2 -translate-x-1/2 w-96 z-50">
          <Alert variant="destructive">
            <AlertTitle className="flex items-center justify-center gap-1">
              <CircleAlert size={17} /> Error!
            </AlertTitle>
            <AlertDescription className="flex justify-center mt-1">
              {alert === "emptyLink" ? "Por favor, preencha o campo." : "Por favor, forneça um link válido."}
            </AlertDescription>
          </Alert>
        </motion.div>
      )}
      ...
      {state === "Form" && (
        <div className="relative z-10 max-w-2xl w-full text-center space-y-10">
          <div className="space-y-4">
            ...
            <h2 className="text-4xl font-bold text-gray-300">favoritas em segundos</h2>
            <p className="text-lg text-gray-400 mt-4">
              Cole o link de qualquer playlist pública do <span className="text-green-400 font-semibold">Spotify</span>.
            </p>
          </div>
      ...
      {state === "Preview" && playlist && <MusicPreview playlist={playlist} />}
    </div>
  );
}
