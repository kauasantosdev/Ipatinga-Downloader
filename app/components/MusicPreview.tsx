...
export default function MusicPreview({ playlist }: MusicPreviewProps) {
  ...
  async function startJobAndListen() {
    ...
    try {
      const resp = await axios.post(
        "https://ipatinga-downloader-server.onrender.com/download/start",
        { tracks: tracksToSend },
  ...
  function listenEvents(id: string) {
    ...
    evtRef.current = new EventSource(`https://ipatinga-downloader-server.onrender.com/events/${id}`);
  ...
  async function downloadZip(downloadUrl: string) {
    try {
      const fullUrl = downloadUrl.startsWith("http") ? downloadUrl : `https://ipatinga-downloader-server.onrender.com${downloadUrl}`;
